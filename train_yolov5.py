...
with open('pcb_defect\\DeepPCB-master\\PCBData\\test.txt') as f:
    for line in f.readlines():
        test.append(line)

with open('pcb_defect\\DeepPCB-master\\PCBData\\trainval.txt') as f1:
...
for k in tqdm(datasets):
    ...
    for i in tqdm(k):
        ...
        for j in i.split(" "):
            print_buffer = []
            if '.jpg' in j:
                path_jpg = 'pcb_defect/DeepPCB-master/PCBData/' + j
            ...
            if '.txt' in j:
                path_txt = 'pcb_defect/DeepPCB-master/PCBData/' + j.strip('\n')
...
final_df.to_csv('tmp\\meta.csv', index=False)
